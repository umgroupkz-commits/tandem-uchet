...
## Уточнения при реализации (05.09.2026)

1. Ограничение `check (netto <= brutto)` — не в базе, а в `chart_save` и только для ручного
   ввода: данные iiko содержат исключение (1 строка), и жёсткое ограничение в схеме
   заблокировало бы перенос.
2. Кандидаты на перенос — все активные `dish`/`prepared` с `iiko_id` (2 191 позиция); у 2 из
   них карт в iiko нет — это ожидаемо, не ошибка.
3. `estimatedPurchasePrice` в iiko у Тандема пуст везде; запасного источника цен нет —
   `iiko_estimate` как источник не используется.
4. При равной дате начала двух версий побеждает первая записанная (идемпотентность повторного
   переноса той же выгрузки), а не версия, пришедшая позже. Перенос не удаляет карты со
   `source='office'` — правленные в бэк-офисе карты остаются нетронутыми.
5. `office_chart_get` принимает параметр `id` (id версии карты), а не `chart_id`.
6. У `iiko-migrate-charts.mjs` нет и не нужен флаг `--from-cache` — кэш в `data/iiko/charts/`
   используется всегда, если он есть.
7. Фактические числа переноса: карт создано **2 314** у **2 180** блюд, строк карт —
   **14 706**, неизвестных ингредиентов — **0**. Цен закупа перенесено **312** — накладные
   доступны только по предприятию Енешки (`incoming_invoice/list` у трёх остальных
   предприятий отвечает HTTP 500 на стороне iiko). Себестоимость посчитана у **603** из
   **1 644** продаваемых блюд (**36,7 %**). Один ингредиент без цены — «Вода ПФ» (код 10) —
   закрывает 562 блюда; после ручного ввода его цены покрытие поднимается примерно до
   **71 %**. Критерий готовности 9.2 (80 %) до ручного дозаполнения цен не достигнут — это
   ожидалось: источника цен закупа по трём предприятиям из четырёх нет.
