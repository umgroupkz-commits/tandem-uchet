-- Ускорение items_lookup_list: ветка «по названию» считала номенклатуру заново для каждой
-- строки файла (коррелированный count по lower(name)) — 15 мс на строку, лист оборотки
-- на 1 000 строк упирался в statement timeout. Теперь уникальные названия собираются одним
-- проходом (group by lower(name) having count(*) = 1) и соединяются хешем: 2 000 ключей ~45 мс.
-- Раздел номенклатуры пересоздаётся целиком — тело взято из 0019 (сверено md5 с базой).
