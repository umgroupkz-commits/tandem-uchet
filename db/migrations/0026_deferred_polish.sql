-- Отложенные замечания подпроекта 1 (docs/superpowers/specs/2026-09-05-uchet-core-deferred.md):
-- «%» и «_» в поиске ищутся буквально; контрагент находится по части БИН и по телефону в любом
-- написании (+7/8, скобки, дефисы); цену по умолчанию можно очистить; группа не входит сама в
-- себя и в свою подгруппу; смена роли обрывает сессии пользователя (триггер — диспетчер не
-- пересоздаётся); индекс сессий по пользователю.
-- office_counteragents — из 0006 (с 0008 security invoker), office_nomenclature — из 0020; md5 сверены.

create index if not exists sessions_user_idx on tandem.sessions (user_id);

-- Экранирование для LIKE/ILIKE (экранирующий символ по умолчанию — обратная косая черта).
create or replace function tandem.like_escape(p text) returns text
language sql immutable as $$
  select replace(replace(replace(p, '\', '\\'), '%', '\%'), '_', '\_')
$$;

-- Смена роли меняет права, а сессия хранит пользователя, не права: выкидываем, пусть войдёт заново.
create or replace function tandem.users_role_sessions() returns trigger
language plpgsql as $$
begin
...
drop trigger if exists users_role_sessions on tandem.users;
create trigger users_role_sessions after update of role on tandem.users
  for each row execute function tandem.users_role_sessions();
