-- Расход для 1С (просьба Светланы 23.09.2026): бухгалтер списывает продукты в 1С «на основании
-- продаж», а номенклатура 1С и учёта разная. Справочник позиций 1С (tandem.catalog_1c, из материальной
-- ведомости), соответствие у позиции учёта (items.code_1c, items.k_1c — единиц 1С в одной нашей) и отчёт
-- stock_1c_report: расход продаж и производства за период в позициях и единицах 1С.
-- Справочник и соответствия загружены из сверки апреля 2026 отдельным скриптом (данные клиента — не в репозитории).
-- office_stock_ext — тело из 0028 + три действия.
...
alter table tandem.items add column if not exists code_1c text references tandem.catalog_1c(code);
alter table tandem.items add column if not exists k_1c numeric;
...
do $$ begin
  if exists (select 1 from pg_roles where rolname = 'service_role') then
    grant all on tandem.catalog_1c to service_role;
  end if;
end $$;
