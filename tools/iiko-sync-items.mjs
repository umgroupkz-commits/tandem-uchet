...
//
// ВНИМАНИЕ. После переноса справочников (tools/iiko-migrate.mjs) основным путём является
// перенос; этот скрипт — только для догрузки продаваемых групп. Правки бэк-офиса
// (название, группа, единица) он перезапишет: его RPC tandem_sync_items обновляет строку
// по коду, не глядя на source.
//
